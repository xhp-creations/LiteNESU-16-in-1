...
byte ppu_addr_latch;
int ppuDrawBack; // Turn off backgrounds on certain games to keep them from crashing, need to find out the cause of crash

void setDrawBack(int drawBack)
{
	ppuDrawBack = drawBack;
}
...
void ppu_cycle()
{
    ...
    if (ppu_shows_background()) {
		...
		if (ppuDrawBack) { // Turning off for some games keeps them from crashing, but turns off background too
...
{
    // nes_set_bg_color(color);  // Never called, not sure why it's in original code
}
