...
#include "litenesu/draw.h"
#include "litenesu/cpu.h"
#include "litenesu/mem.h"
#include "litenesu/ppu.h"
#include "litenesu/hal.h"
#include "litenesu/mmc.h"
...
int Menu_Main()
{
	...
	while(1)
	{
		...
		buttonValue = vpad_data.btns_h; // Report button value to HAL
...
int fce_load_rom(const unsigned char *rom)
{
    ...
    u8 *buf = (u8 *)malloc(prg_size + (fce_rom_header.chr_block_count * 0x2000) + 1);
